...
set_command_line_rp <- function(){
  ...
  if(os%in%"linux"){
    if(grepl("amd64",Sys.info()[["machine"]])&grepl("linux-gnu", R.version$os)){
      sysCommand <- c("cp",paste(normalizePath(paste0(sourcePath,"/linux_AMD_x86_64gnu/rp_x86_64.dms")), normalizePath(paste0(execPath,"/rp"), mustWork = FALSE)),"chmod", paste("a+x ",normalizePath(paste0(execPath,"/rp"), mustWork = FALSE)))
