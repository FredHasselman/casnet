...
#' @param rescaleDimensions If the dimensions are not on the same scale, transform by using `"standardise"` or `"unit"`, otherwise use `"none"` (default = `"unit"`)
...
rn_phases <- function(RN, maxPhases = 5, minStatesinPhase = 1, maxStatesinPhase = NROW(RN), removeSingularities = FALSE, standardise = c("none","mean.sd","median.mad","unit")[4], returnGraph = FALSE, doPhasePlot = FALSE,  phaseColours = NULL, doSpiralPlot = FALSE, showEpochLegend = TRUE, epochColours = NULL, epochLabel = "Phase", excludeOther = FALSE, excludeNorec = TRUE){
  ...
  out <- arrange(out,states_time)

  # Plots
  if(any(doPhasePlot,doSpiralPlot)){

    out_long <- pivot_longer(out, cols = (("states_dist2maxState"%ci%out)+1):NCOL(out), names_to = "Dimension", values_to = "Value")

    if(excludeOther){
      out_long <- out_long %>% filter(phase_name != "Other")
    }
    if(excludeNorec){
      out_long <- out_long %>% filter(phase_name != "No recurrence")
    }

    out_long <- arrange(out_long, as.numeric(out_long$phase_number))
...
rn_transition <- function(phaseSequence, threshold = NA, doMatrixPlot = TRUE, doNetworkPlot = TRUE, excludeOther = FALSE, excludeNorec = TRUE, returnGraph = FALSE){
  ...
  if(excludeOther){
    phaseSequence <- phaseSequence %>% as_tibble() %>% filter(phaseSequence != "Other")
    phaseSequence <- phaseSequence$value
  }
  if(excludeNorec){
    phaseSequence <- phaseSequence %>% as_tibble() %>% filter(phaseSequence != "No recurrence")
    phaseSequence <- phaseSequence$value
  }
  ...
  if(doMatrixPlot){
    ...
    mp <-  ggplot(df, aes(x = to_name,
                          y = from_name)) +
      geom_tile(aes(fill = freq),colour = "black",size = 0.4) +
      geom_text(aes(label = ifelse(is.na(freq),"0",signif(freq,3)))) +
