...
```{r}
dc.case <- dc_win(df, win=12, scale_min=0, scale_max=100, doPlot = TRUE)
```

By setting `colOrder = NA`, we can order the output columns based on mean dynamic complexity, resulting in a more comprehensive diagram:
```{r}
dc.case <- dc_win(df, win=12, scale_min=0, scale_max=100, doPlot = TRUE, colOrder = NA)
```
...
plotDC_ccp(df_ccp = ccp.case, win = 12, colOrder = NA, title = "Cumulative Complexity Peak Plot")
```
...
```{r}
var.case <- var_win(df, win=12, doPlot = TRUE, colOrder = NA)
```
...
...and for the full dataset:
```{r}
ac.case <- ac_win(df, win=28, doPlot = TRUE, colOrder = NA)
```
