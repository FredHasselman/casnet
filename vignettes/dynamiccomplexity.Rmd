...
df_lvl_hrs   <- ts_levels(df$hours)
df_lvl_diff  <- ts_levels(df$difficult)
df_lvl_unsat <- ts_levels(df$unsatisfy)

df_lvl_hrsNA   <- ts_levels(df$hours[idNA])
df_lvl_diffNA  <- ts_levels(df$difficult[idNA])
df_lvl_unsatNA <- ts_levels(df$unsatisfy[idNA])
...
plotDC_lvl(df_ccp = ccp.caseNA, df_win = dc.caseNA$df, df_lvl = df_lvl_hrsNA, useTimeVector = dateTimesS[idNA], win = win, subtitle = "Remove NA", levelName = "Hours sleep")
...
plotDC_lvl(df_ccp = ccp.caseNA, df_win = dc.caseNA$df, df_lvl = df_lvl_diffNA, useTimeVector = dateTimesS[idNA], win = win, subtitle = "Remove NA", levelName = "Difficulty sleeping")
...
plotDC_lvl(df_ccp = ccp.caseNA, df_win = dc.caseNA$df, df_lvl = df_lvl_unsatNA, useTimeVector = dateTimesS[idNA], win = win, subtitle = "Remove NA", levelName = "Unsatisfactory sleep")
...
Schiepek, G. (2003). A Dynamic Systems Approach to Clinical Case Formulation. *European Journal of Psychological Assessment, 19*, 175-184. httpd://doi.org/10.1027//1015-5759.19.3.175
