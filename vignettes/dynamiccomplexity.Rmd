...
Often, the purpose of detecting early warning signals is to predict changes in a state variable, for example a test for depressive symptoms, or, a broad assesment by a therapist, measured once a week. Here we do not have clear apriori state variables, we'll use the measure 'Feeling Positive' as it can be considered indicative for both major depressive disorder and generalized anxiety disorder. We'll assume in this example that 'Feeling Positive' indicate a "global sate" of the system.


### Detecting periods of relative stability in state variables recursive partitioning
...
One can see that there appears to be a sudden shift in feeling positive around half way of the measurement period.
...
lvl <- ts_levels(out.cart$positive, minDataSplit = 12, minLevelDuration = 28, changeSensitivity = 0.05)

# ggplot likes the long data format
df_state <- data.frame(days = dateTimesS,
                       positive = out.cart$positive) %>% 
...
And this is what it looks like without imputation.
```{r}
####
lvl.NA <- ts_levels(out.NA$positive, minDataSplit = 12, minLevelDuration = 28, changeSensitivity = 0.05)

# ggplot likes the long data format
df_state.NA <- data.frame(days = dateTimess.NA ,
                       positive = out.NA$positive) %>% 
  tidyr::gather(key = variable, value = value, -days, factor_key = TRUE)

df_state.NA$level <-  
  data.frame(days = dateTimess.NA ,
             positive_lvl = lvl.NA$pred$p) %>% 
  tidyr::gather(key = variable_lvl, value = level, -days, factor_key = TRUE) %>%
  dplyr::select("level") %>%
  unlist(.)
  
ggplot(df_state.NA, aes(x = days)) +
  geom_line(aes(y = value), linetype = 2) +
  geom_point(aes(y = value), pch=16, size=2, colour = "steelblue") +
  geom_line(aes(y = level), colour = "red3") +
  facet_grid(variable~., scales = "free_y") +
...
# NA removed 
dc.caseNA <- dc_win(out.NA, win = win, scale_min=0, scale_max=100, doPlot = TRUE, colOrder = TRUE, doPlotF = TRUE, doPlotD = TRUE)
```
...
One can obtain a dynamic complexity profile by averaging the dynamic complexity scores over all items.
```{r}
mean_DC <- rowMeans(as.matrix(dc.caseIMP))
...
One could look for a single variable that best predicts level changes, e.g. by looking at crosscorrelation functions, or by extracting peak-complexity times and gain/loss events can average the time course of dynamic complexity in a number of ways.

Here we make a plot of the state variable, the levels and the dynamic complexity. 
```{r ews2}

out.cart$measure <- (1:nrow(out.cart))
out.cart$lvl <- lvl$pred$p
out.cart %>% 
  mutate(Level=ts_standardise(lvl), StateVariable=ts_standardise(positive), DynamicComplexity=ts_standardise(mean_DC)) %>% 
  select(measure,StateVariable,DynamicComplexity, Level) %>% 
  gather(key=variable,value=value,-measure) %>%
  ggplot(aes(x=measure, y=value, colour=variable)) + geom_line() + theme_bw()
```

Here we use the function `plotDC_lvl` to look for high dynamic complexity in relation to shifts in the state variable.
```{r}
...
df_lvl   <-  ts_levels(out.cart$positive, minDataSplit = 12, minLevelDuration = 28, changeSensitivity = 0.05)
df_lvl_NA <- ts_levels(out.NA$positive, minDataSplit = 12, minLevelDuration = 28, changeSensitivity = 0.05)

plotDC_lvl(df_ccp = ccp.caseIMP, df_win = dc.caseIMP, df_lvl = df_lvl, useTimeVector = dateTimesS, win = win, subtitle = "CART", levelName = "Feeling Positive")

plotDC_lvl(df_ccp = ccp.caseNA, df_win = dc.caseNA, df_lvl = df_lvl_NA, useTimeVector = dateTimesS[idNA], win = win, subtitle = "Remove NA", levelName = "Feeling Positive")
```
