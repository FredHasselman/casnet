...
Dynamic assessment, for example, ecological momentary assessment (EMA), is used more frequently in scientific studies on mental health. Process monitoring with dynamic assessment can empower patients and benefits self-insight (van Os et al., 2017). Additionally, EMA time series can be analysed and the results can be used to inform the treatment process. 
...
It's a good practice to always compare results based on imputation to non-imputed data, so we'll create two datasets:
